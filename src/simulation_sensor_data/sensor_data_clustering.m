...
for i = 1:height(data)
    ...
    for j = 1:size(objects,1) 
        ...
        elseif (objects{j,1}.SensorIndex == 4) 
            if (preSensor ~= 4)
                ...
                ave4_vy_ = objects{j,1}.Measurement(5,1);
                preSensor = 4;
            else
                if (abs(objects{j,1}.Measurement(1,1) - ave4_dx_) <= EGO_VEHICLE_LENGTH && abs(objects{j,1}.Measurement(2,1) - ave4_dy_) <= EGO_VEHICLE_WIDTH)
                    ...
                    ave4_vy_ = (ave4_vy_ + objects{j,1}.Measurement(5,1))/(2.0);
                end
            end
        end
    end
    ...
    data_output_(i,6) = ave2_dx_;
    data_output_(i,7) = ave2_dy_;
    data_output_(i,8) = ave2_vx_;
    data_output_(i,9) = ave2_vy_;
    data_output_(i,10) = ave3_dx_;
    data_output_(i,11) = ave3_dy_;
    data_output_(i,12) = ave3_vx_;
    data_output_(i,13) = ave3_vy_;
    data_output_(i,14) = ave4_dx_;
    data_output_(i,15) = ave4_dy_;
    data_output_(i,16) = ave4_vx_;
    data_output_(i,17) = ave4_vy_;
end
