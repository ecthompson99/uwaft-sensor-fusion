...
std::vector<RadarObject> DataAssociation::filter_radar(const common::radar_object_data& recvd_data) {
  ...
  for (size_t r_index = 0; r_index < RADAR_OBJ; r_index++) {
    // dx and dy limits
    if (recvd_data.radar_dx[r_index] > MAX_DX_RADAR || recvd_data.radar_dx[r_index] < MIN_DX ||
        recvd_data.radar_dx[r_index] > MAX_DX || abs(recvd_data.radar_dy[r_index]) > MAX_DY)
      continue;
    ...
    // printf("Success filtering radar data: %f, %f, %f, %f", recvd_data.radar_dx[r_index],
    // recvd_data.radar_dy[r_index], recvd_data.radar_vx[r_index], recvd_data.radar_vy[r_index]);
...
std::vector<MobileyeObject> DataAssociation::filter_me(const common::mobileye_object_data& recvd_data) {
  ...
  for (size_t me_index = 0; me_index < ME_OBJ; me_index++) {
    // dx and dy threshold
    if (recvd_data.me_dx[me_index] < MIN_DX_ME || recvd_data.me_dx[me_index] < MIN_DX ||
        recvd_data.me_dx[me_index] > MAX_DX || abs(recvd_data.me_dy[me_index]) > MAX_DY)
      continue;
...
void DataAssociation::sensor_radar_data_obj_callback(const common::radar_object_data& recvd_data) {
  ...
  if (FRONT_RADAR) {  // && LEFT_CORNER_RADAR && RIGHT_CORNER_RADAR){
    ...
    if (client.call(srv)) {
      ...
      ROS_ERROR(
          "Failed to call service, but continuing to the already stored potential objects, maybe it matches up "
          "there!?");
    }
