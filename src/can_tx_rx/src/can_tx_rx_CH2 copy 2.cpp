...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    if (now.toSec() - mem1.toSec() > 0.1) {  // try commenting out
      ...
      stat1 = canReadSpecific(hnd, 0x500, &can_data, &dlc, &flag, &time);
      stat2 = canReadSpecific(hnd, 0x502, &can_data, &dlc, &flag, &time);
      if  ((stat1 == canOK)){ // && (id == 1280 || id == 1282)) { // starter bit
        pub_data = true;
      ...
      stat = canRead(hnd, &id, &can_data, &dlc, &flag, &time);
      if  ((stat == canOK) && (id == 1985 || id == 1958 || id == 1879 || id == 1957)) { // diagnostic responses
      ...
      stat = canRead(hnd, &id, &can_data, &dlc, &flag, &time);
      if  ((stat == canOK) && (id > 1604 && id < 1659)) { // target tracking
        std::cout << "Case 2" << std::endl;
        if (id % 10 == 5 || id % 10 == 7) { // a frame in dbc (all ids for targets end with a 5 if they are radar 1, or 7 if they are  radar 2 
        ...
        else if (id % 10 == 6 || id % 10 == 8) {  // frame b in dbc
        ...
        target_object_num = (id - 1600 - (id % 10)) / 10;  // takes the target object number based on the defined id

      }

      stat = canRead(hnd, &id, &can_data, &dlc, &flag, &time);
      if  ((stat == canOK) && (id == 1665 || id == 1667 || id == 1280 || id == 1282 || id == 1670 || id == 1672)) { // ender, starter, and statuses messages
      ...
      stat = canRead(hnd, &id, &can_data, &dlc, &flag, &time);
      if  ((stat == canOK) && (id > 1284 && id < 1597)) { // radar A and B object frames
        std::cout << "Case 4" << std::endl;
        obj_num = (id - 1280 - (id % 10)) / 10;  // takes the tracked object number based on the defined id 
        ...
        else if (id % 10 == 6 || id % 10 == 8) { // takes the target object number based on the defined id
      ...
      stat1 = canReadSpecific(hnd, 0x681, &can_data, &dlc, &flag, &time);
      stat2 = canReadSpecific(hnd, 0x683, &can_data, &dlc, &flag, &time);
      
      if  ((stat1 == canOK) && (pub_data)) { // message must end with the ender bit and have started with an end bit
