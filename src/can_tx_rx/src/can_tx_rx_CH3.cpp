...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    if (canOK == stat) {
          ...
          switch (frame_num) {
            case 1:  // a frame
              ...
              target_info.target_mess_aconsist_bit_is_in_range =
                  radar_target_a_radar_target_mess_aconsist_bit_is_in_range(r_target_a.radar_target_mess_aconsist_bit);
              break;
            case 2:
              ...
              target_info.target_mess_bconsist_bit_is_in_range =
                  radar_target_b_radar_target_mess_bconsist_bit_is_in_range(r_target_b.radar_target_mess_bconsist_bit);
              break;
          }
          break;
      ...
      uint16_t int16_2bit;
      size_t size = 8u;
      // goal: ensure that there are 7 entries in the unsigned 8 bit integer, grouping smaller 1-4 bits signals or
      // breaking up 16 bit signals as necessary
      // to do: research if crc requires bits to be properly ordered as shown in dbc and if there is an easier way to do
      // so
