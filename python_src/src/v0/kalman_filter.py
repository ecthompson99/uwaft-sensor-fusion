...
from src.v0.config import FilteredObject
...
class KFNode:
    ...
    def me_callback(self, obj):
        ...
        result.id = obj.id
        result.obj_lane, result.obj_path = self.determine_lane_me(result.dy, result.dx)
    ...
    def radar_callback(self, obj):
        ...
        result.dx, result.dy, result.vx, result.vy = self.objects[obj.id].x
        result.id = obj.id
        result.obj_lane, result.obj_path = self.determine_lane(result.dy)
        result.timestamp = obj.timestamp
        result.count = 30
