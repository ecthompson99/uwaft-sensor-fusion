...
[tracked_A, tracked_count, tracked_obj] = load_rosbag();
...
trackedObjectMarker = '+'; %Plot marker for tracked objects
...
for i = 1:size(tracked_obj,1)
    trackedObjects{i} = SFObject(tracked_obj(i), tracked_pos{i}, trackedObjectMarker, ...
        contrastingColors(i, :), tracked_count(i));
end

r = BEPlotter(trackedObjects, timestepSize);
